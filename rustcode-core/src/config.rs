...
pub fn default_lang_config() -> Configuration {
    rustcode_loader::config::default_lang_config()
        .try_into()
...
pub fn user_lang_config() -> Result<Configuration, toml::de::Error> {
    rustcode_loader::config::user_lang_config()?.try_into()
}
...
pub fn user_lang_loader() -> Result<Loader, LanguageLoaderError> {
    let config: Configuration = rustcode_loader::config::user_lang_config()
