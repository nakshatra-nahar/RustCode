...
pub use dap::*;
use rustcode_event::status;
use rustcode_stdx::{
    path::expand_tilde,
    rope::{self, RopeSliceExt},
};
use rustcode_vcs::{FileChange, Hunk};
pub use lsp::*;
use tui::{
    text::Span,
    widgets::{Cell, Row},
...
impl<'a> Context<'a> {
    ...
    pub fn callback<T, F>(
        &mut self,
        call: impl Future<Output = rustcode_lsp::Result<serde_json::Value>> + 'static + Send,
        callback: F,
    ) where
...
use rustcode_view::{align_view, Align};
...
use rustcode_core::movement::{move_horizontally, move_vertically};
...
fn align_selections(cx: &mut Context) {
    use rustcode_core::visual_coords_at_pos;
...
/// Opens the given url. If the URL points to a valid textual file it is open in rustcode.
...
fn open_url(cx: &mut Context, url: Url, action: Action) {
    ...
    // we attempt to open binary files - files that can't be open in rustcode - using external
...
fn copy_selection_on_line(cx: &mut Context, direction: Direction) {
    use rustcode_core::{pos_at_visual_coords, visual_coords_at_pos};
...
fn global_search(cx: &mut Context) {
    ...
    impl ui::menu::Item for FileResult {
        ...
        fn format(&self, current_path: &Self::Data) -> Row {
            let relative_path = rustcode_stdx::path::get_relative_path(&self.path)
                .to_string_lossy()
                .into_owned();
            if current_path
                .as_ref()
    ...
    ui::raw_regex_prompt(
        ...
        move |cx, _, input, event| {
            ...
            if let Ok(matcher) = RegexMatcherBuilder::new()
                ...
                .build(input)
            {
                let search_root = rustcode_stdx::env::current_working_dir();
                ...
                std::thread::spawn(move || {
                    ...
                    walk_builder
                        .add_custom_ignore_filename(rustcode_loader::config_dir().join("ignore"));
                    walk_builder.add_custom_ignore_filename(".rustcode/ignore");
...
fn file_picker_in_current_directory(cx: &mut Context) {
    let cwd = rustcode_stdx::env::current_working_dir();
    if !cwd.exists() {
        cx.editor
            .set_error("Current working directory does not exist");
        return;
...
pub mod insert {
    ...
    use rustcode_core::auto_pairs;
    use rustcode_view::editor::SmartTabConfig;
    ...
    pub fn insert_char(cx: &mut Context, c: char) {
        ...
        rustcode_event::dispatch(PostInsertChar { c, cx });
    }
...
fn format_selections(cx: &mut Context) {
    use rustcode_lsp::{lsp, util::range_to_lsp_range};
    ...
    let edits = tokio::task::block_in_place(|| rustcode_lsp::block_on(future)).unwrap_or_default();

    let transaction =
        rustcode_lsp::util::generate_transaction_from_edits(doc.text(), edits, offset_encoding);
...
where
    F: Fn(&rustcode_core::Syntax, RopeSlice, Selection) -> Selection + 'static,
{
    let motion = move |editor: &mut Editor| {
        let (view, doc) = current!(editor);
...
fn shell_impl(shell: &[String], cmd: &str, input: Option<Rope>) -> anyhow::Result<Tendril> {
    tokio::task::block_in_place(|| rustcode_lsp::block_on(shell_impl_async(shell, cmd, input)))
}
    ...
    let output = if let Some(mut stdin) = process.stdin.take() {
        let input_task = tokio::spawn(async move {
            if let Some(input) = input {
                rustcode_view::document::to_writer(&mut stdin, (encoding::UTF_8, false), &input)
                    .await?;
            }
            anyhow::Ok(())
        });
...
fn replay_macro(cx: &mut Context) {
    ...
    {
        match rustcode_view::input::parse_macro(&keys) {
            Ok(keys) => keys,
            Err(err) => {
