...
| `auto-save` | Enable automatic saving on the focus moving away from rustcode. Requires [focus event support](https://github.com/rustcode-editor/rustcode/wiki/Terminal-Support) from your terminal | `false` |
...
| `workspace-lsp-roots` | Directories relative to the workspace root that are treated as LSP roots. Should only be set in `.rustcode/config.toml` | `[]` |
...
[^2]: You may also have to activate them in the LSP config for them to appear, not just in rustcode. Inlay hints in rustcode are still being improved on and may be a little bit laggy/janky under some circumstances. Please report any bugs you see so we can fix them!
...
not visible in the rustcode file picker and global search.
...
Additionally, you can use rustcode-specific ignore files by creating a local `.rustcode/ignore` file in the current workspace or a global `ignore` file located in your rustcode config directory:
- Linux and Mac: `~/.config/rustcode/ignore`
- Windows: `%AppData%\rustcode\ignore`
