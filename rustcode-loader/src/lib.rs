...
use rustcode_stdx::{env::current_working_dir, path};
...
/// 3. `rustcode_RUNTIME` (if environment variable is set)
/// 4. `rustcode_DEFAULT_RUNTIME` (if environment variable is set *at build time*)
/// 5. subdirectory of path to rustcode executable (always included)
///
...
fn prioritize_runtime_dirs() -> Vec<PathBuf> {
    ...
    if let Ok(dir) = std::env::var("rustcode_RUNTIME") {
    ...
    if let Some(dir) = std::option_env!("rustcode_DEFAULT_RUNTIME") {
...
pub fn workspace_config_file() -> PathBuf {
    find_workspace().0.join(".rustcode").join("config.toml")
}
...
/// and returns the first directory that contains either `.git` or `.rustcode`.
/// If no workspace was found returns (CWD, true).
/// Otherwise (workspace, false) is returned
pub fn find_workspace() -> (PathBuf, bool) {
    let current_dir = current_working_dir();
    for ancestor in current_dir.ancestors() {
        if ancestor.join(".git").exists() || ancestor.join(".rustcode").exists() {
