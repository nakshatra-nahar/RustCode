...
pub use rustcode_core::diagnostic::LanguageServerId;
...
pub enum Error {
    ...
    Unhandled,
    #[error(transparent)]
    ExecutableNotFound(#[from] rustcode_stdx::env::ExecutableNotFoundError),
    #[error(transparent)]
...
pub mod util {
    use super::*;
    use rustcode_core::line_ending::{line_end_byte_index, line_end_char_index};
    use rustcode_core::{chars, RopeSlice, SmallVec};
    use rustcode_core::{diagnostic::NumberOrString, Range, Rope, Selection, Tendril, Transaction};
    ...
    pub fn diagnostic_to_lsp_diagnostic(
        doc: &Rope,
        diag: &rustcode_core::diagnostic::Diagnostic,
        ...
        let new_tags: Vec<_> = diag
            ...
            .iter()
            .map(|tag| match tag {
                rustcode_core::diagnostic::DiagnosticTag::Unnecessary => {
                    lsp::DiagnosticTag::UNNECESSARY
                }
                rustcode_core::diagnostic::DiagnosticTag::Deprecated => lsp::DiagnosticTag::DEPRECATED,
            })
            .collect();
    ...
    pub fn lsp_pos_to_pos(
        ...
        //
        // FIXME: rustcode does not fully comply with the LSP spec for line terminators.
        // The LSP standard requires that line terminators are ['\n', '\r\n', '\r'].
        // Without the unicode-linebreak feature disabled, the `\r` terminator is not handled by rustcode.
        // With the unicode-linebreak feature, rustcode recognizes multiple extra line break chars
    ...
    pub fn generate_transaction_from_snippet(
        ...
        let (transaction, mut selection) = Transaction::change_by_selection_ignore_overlapping(
            ...
            |replacement_start, replacement_end| {
                ...
                let indent_level = rustcode_core::indent::indent_level_for_line(
        ...
        for (range, (tabstop_anchor, tabstops)) in selection.into_iter().zip(selection_tabstops) {
            ...
            // expand the selection to cover the tabstop to retain the rustcode selection semantic
    ...
    pub fn generate_transaction_from_edits(
        ...
        if edits.len() == 1 {
            ...
            if is_document_replacement {
                ...
                return rustcode_core::diff::compare_ropes(doc, &new_text);
            }
        }
...
pub struct Registry {
    ...
    syn_loader: Arc<ArcSwap<rustcode_core::syntax::Loader>>,
    pub incoming: SelectAll<UnboundedReceiverStream<(LanguageServerId, Call)>>,
    pub file_event_handler: file_event::Handler,
}

impl Registry {
    pub fn new(syn_loader: Arc<ArcSwap<rustcode_core::syntax::Loader>>) -> Self {
        Self {
    ...
    /// See rustcode_view::editor::Editor::refresh_language_servers
    pub fn restart(
        &mut self,
...
fn start_client(
    ...
    let (workspace, workspace_is_cwd) = rustcode_loader::find_workspace();
...
pub fn find_lsp_workspace(
    ...
    let mut file = if file.is_absolute() {
        ...
        let current_dir = rustcode_stdx::env::current_working_dir();
