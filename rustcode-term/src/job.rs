use rustcode_event::status::StatusMessage;
use rustcode_event::{runtime_local, send_blocking};
...
impl Jobs {
    ...
    pub fn new() -> Self {
        ...
        let status_messages = rustcode_event::status::setup();
        Self {
        ...
        if j.wait {
            ...
            tokio::spawn(async move {
                match j.future.await {
                    ...
                    Ok(None) => (),
                    Err(err) => rustcode_event::status::report(err).await,
                }
            });
        }
    }
