...
use rustcode_core::{syntax::LanguageServerFeature, text_annotations::InlineAnnotation, Selection};
...
/// to make them easier to navigate. rustcode does not display these  headings to the user.
        ...
        .filter_map(|language_server| {
            ...
            let code_action_context = lsp::CodeActionContext {
                diagnostics: doc
                    ...
                    .filter(|&diag| {
                        selection_range
                            .overlaps(&rustcode_core::Range::new(diag.range.start, diag.range.end))
                    })
                ...
                match &action.lsp_item {
                    ...
                    lsp::CodeActionOrCommand::CodeAction(code_action) => {
                        ...
                        if code_action.edit.is_none() || code_action.command.is_none() {
                            ...
                            {
                                if let Ok(response) = rustcode_lsp::block_on(future) {
...
fn goto_impl(
    ...
    let cwdir = rustcode_stdx::env::current_working_dir();
...
where
    ...
    F: Future<Output = rustcode_lsp::Result<serde_json::Value>> + 'static + Send,
{
    let (view, doc) = current!(cx.editor);
...
pub fn rename_symbol(cx: &mut Context) {
    fn get_prefill_from_word_boundary(editor: &Editor) -> String {
        ...
        if primary_selection.len() > 1 {
            ...
            use rustcode_core::textobject::{textobject_word, TextObject};
...
fn compute_inlay_hints_for_view(
    ...
    let range = rustcode_lsp::util::range_to_lsp_range(
        doc_text,
        rustcode_core::Range::new(first_char_in_range, last_char_in_range),
    ...
    let callback = super::make_job_callback(
        ...
        move |editor, _compositor, response: Option<Vec<lsp::InlayHint>>| {
            ...
            for hint in hints {
                let char_idx =
                    match rustcode_lsp::util::lsp_pos_to_pos(doc_text, hint.position, offset_encoding)
                    {
                        Some(pos) => pos,
