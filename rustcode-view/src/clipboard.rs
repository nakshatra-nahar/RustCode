...
pub mod provider {
    ...
    impl FallbackProvider {
        pub fn new() -> Self {
            #[cfg(feature = "term")]
            log::debug!(
                "No native clipboard provider found. Yanking by OSC 52 and pasting will be internal to rustcode"
            );
            #[cfg(not(feature = "term"))]
            log::warn!(
                "No native clipboard provider found! Yanking and pasting will be internal to rustcode"
            );
            Self {
                buf: String::new(),
    ...
    pub mod command {
        ...
        impl Config {
            fn execute(&self, input: Option<&str>, pipe_output: bool) -> Result<Option<String>> {
                ...
                // Fix for https://github.com/rustcode-editor/rustcode/issues/5424
                if cfg!(unix) {
