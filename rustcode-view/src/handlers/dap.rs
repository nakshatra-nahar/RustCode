...
use rustcode_core::Selection;
use rustcode_dap::{self as dap, Client, ConnectionType, Payload, Request, ThreadId};
use rustcode_lsp::block_on;
use log::warn;
use std::fmt::Write;
...
// general utils:
pub fn dap_pos_to_pos(doc: &rustcode_core::Rope, line: usize, column: usize) -> Option<usize> {
...
pub fn jump_to_stack_frame(editor: &mut Editor, frame: &rustcode_dap::StackFrame) {
    let path = if let Some(rustcode_dap::Source {
        path: Some(ref path),
        ..
    }) = frame.source
    {
...
pub fn breakpoints_changed(
    ...
    let source_breakpoints = breakpoints
        .iter()
        .map(|breakpoint| rustcode_dap::SourceBreakpoint {
...
impl Editor {
    pub async fn handle_debugger_message(&mut self, payload: rustcode_dap::Payload) -> bool {
        use dap::requests::RunInTerminal;
        use rustcode_dap::{events, Event};
