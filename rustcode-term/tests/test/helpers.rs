...
use rustcode_core::{diagnostic::Severity, test, Selection, Transaction};
use rustcode_term::{application::Application, args::Args, config::Config, keymap::merge_keys};
use rustcode_view::{current_ref, doc, editor::LspConfig, input::parse_macro, Editor};
...
impl LineFeedHandling {
    ...
    pub fn apply(&self, text: &str) -> String {
        let line_end = match self {
            LineFeedHandling::Native => rustcode_core::NATIVE_LINE_ENDING,
...
pub async fn test_key_sequence_with_input_text<T: Into<TestCase>>(
    ...
    let (view, doc) = rustcode_view::current!(app.editor);
...
pub fn test_syntax_loader(overrides: Option<String>) -> rustcode_core::syntax::Loader {
    let mut lang = rustcode_loader::config::default_lang_config();

    if let Some(overrides) = overrides {
        let override_toml = toml::from_str(&overrides).unwrap();
        lang = rustcode_loader::merge_toml_values(lang, override_toml, 3);
    }

    rustcode_core::syntax::Loader::new(lang.try_into().unwrap()).unwrap()
}
...
pub fn test_config() -> Config {
    Config {
        ...
        keys: rustcode_term::keymap::default(),
        ..Default::default()
    }
}

pub fn test_editor_config() -> rustcode_view::editor::Config {
    rustcode_view::editor::Config {
        lsp: LspConfig {
            enable: false,
            ..Default::default()
        },
...
pub struct AppBuilder {
    args: Args,
    config: Config,
    syn_loader: rustcode_core::syntax::Loader,
...
impl AppBuilder {
    ...
    pub fn with_file<P: Into<PathBuf>>(
        mut self,
        path: P,
        pos: Option<rustcode_core::Position>,
    ) -> Self {
    ...
    pub fn with_config(mut self, mut config: Config) -> Self {
        let keys = replace(&mut config.keys, rustcode_term::keymap::default());
    ...
    pub fn with_lang_loader(mut self, syn_loader: rustcode_core::syntax::Loader) -> Self {
