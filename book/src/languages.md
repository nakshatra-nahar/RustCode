...
1. In the rustcode source code, which lives in the
   [rustcode repository](https://github.com/rustcode-editor/rustcode/blob/master/languages.toml).
...
2. In your [configuration directory](./configuration.md). This overrides values
   ...
   ```toml
   # in <config_dir>/rustcode/languages.toml
...
3. In a `.rustcode` folder in your project. Language configuration may also be
   overridden local to a project by creating a `languages.toml` file in a
   `.rustcode` folder. Its settings will be merged with the language configuration
...
| `workspace-lsp-roots`     | Directories relative to the workspace root that are treated as LSP roots. Should only be set in `.rustcode/config.toml`. Overwrites the setting of the same name in `config.toml` if set. |
| `persistent-diagnostic-sources` | An array of LSP diagnostic sources assumed unchanged when the language server resends the same set of diagnostics. rustcode can track the position for these diagnostics internally instead. Useful for diagnostics that are recomputed on save.

### File-type detection and the `file-types` key

rustcode determines which language configuration to use based on the `file-types` key
...
When determining a language configuration to use, rustcode searches the file-types
...
The `language-servers` attribute in a language tells rustcode which language servers are used for this language.
