...
use rustcode_core::auto_pairs::AutoPairs;
use rustcode_core::chars::char_is_word;
use rustcode_core::doc_formatter::TextFormat;
use rustcode_core::encoding::Encoding;
use rustcode_core::syntax::{Highlight, LanguageServerFeature};
use rustcode_core::text_annotations::{InlineAnnotation, Overlay};
use rustcode_lsp::util::lsp_pos_to_pos;
use rustcode_stdx::faccess::{copy_metadata, readonly};
use rustcode_vcs::{DiffHandle, DiffProviderRegistry};
...
/// wants them but it shouldn't be rustcode who decides that so we use the most precise positioning.
///
...
use rustcode_lsp::{lsp, Client, LanguageServerId, LanguageServerName};
use url::Url;
...
impl Document {
    ...
    pub fn format(&self) -> Option<BoxFuture<'static, Result<Transaction, FormatterError>>> {
        if let Some((fmt_cmd, fmt_args)) = self
            ...
            .and_then(|formatter| {
                Some((
                    rustcode_stdx::env::which(&formatter.command).ok()?,
                    &formatter.args,
                ))
            })
        {
            ...
            let formatting_future = async move {
                ...
                Ok(rustcode_core::diff::compare_ropes(&text, &Rope::from(str)))
            };
        ...
        let fut = async move {
            ...
            Ok(rustcode_lsp::util::generate_transaction_from_edits(
                &text,
                edits,
                offset_encoding,
            ))
    ...
    > {
        ...
        let path = match path {
            Some(path) => rustcode_stdx::path::canonicalize(path),
            None => {
    ...
    ) -> Option<Arc<rustcode_core::syntax::LanguageConfiguration>> {
        config_loader
    ...
    pub fn reload(
        ...
        // of the encoding.
        let transaction = rustcode_core::diff::compare_ropes(self.text(), &rope);
    ...
    pub fn set_path(&mut self, path: Option<&Path>) {
        let path = path.map(rustcode_stdx::path::canonicalize);
    ...
    pub fn set_language(
        &mut self,
        language_config: Option<Arc<rustcode_core::syntax::LanguageConfiguration>>,
        loader: Option<Arc<ArcSwap<rustcode_core::syntax::Loader>>>,
    ) {
    ...
    pub fn set_selection(&mut self, view_id: ViewId, selection: Selection) {
        ...
        rustcode_event::dispatch(SelectionDidChange {
            doc: self,
            view: view_id,
        })
    }
    ...
    fn apply_impl(
        ...
        if success {
            if emit_lsp_notification {
                rustcode_event::dispatch(DocumentDidChange {
                    doc: self,
                    view: view_id,
                    old_text: &old_doc,
                });
    ...
    pub fn language_servers(&self) -> impl Iterator<Item = &rustcode_lsp::Client> {
    ...
    ) -> impl Iterator<Item = &rustcode_lsp::Client> {
        self.language_config().into_iter().flat_map(move |config| {
            config.language_servers.iter().filter_map(move |features| {
                let ls = &**self.language_servers.get(&features.name)?;
                if ls.is_initialized()
    ...
    pub fn relative_path(&self) -> Option<Cow<Path>> {
        self.path
            .as_deref()
            .map(rustcode_stdx::path::get_relative_path)
    }
    ...
    pub fn position(
        &self,
        view_id: ViewId,
        offset_encoding: rustcode_lsp::OffsetEncoding,
    ) -> lsp::Position {
        let text = self.text();

        rustcode_lsp::util::pos_to_lsp_pos(
            text,
    ...
    pub fn lsp_diagnostic_to_diagnostic(
        ...
        diagnostic: &rustcode_lsp::lsp::Diagnostic,
        language_server_id: LanguageServerId,
        offset_encoding: rustcode_lsp::OffsetEncoding,
    ) -> Option<Diagnostic> {
        use rustcode_core::diagnostic::{Range, Severity::*};
        ...
        use rustcode_core::diagnostic::{DiagnosticTag, NumberOrString};
...
mod test {
    ...
    fn changeset_to_changes_ignore_line_endings() {
        use rustcode_lsp::{lsp, Client, OffsetEncoding};
    ...
    fn test_line_ending() {
        assert_eq!(
            Document::default(Arc::new(ArcSwap::new(Arc::new(Config::default()))))
                .text()
                .to_string(),
            rustcode_core::NATIVE_LINE_ENDING.as_str()
        );
    }
    ...
    macro_rules! encode {
        ($name:ident, $label:expr, $label_override:expr) => {
            ...
            fn $name() {
                ...
                rustcode_lsp::block_on(to_writer(&mut buf, (encoding, false), &text)).unwrap();
