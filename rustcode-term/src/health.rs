...
use rustcode_core::config::{default_lang_config, user_lang_config};
use rustcode_loader::grammar::load_runtime_file;
use rustcode_view::clipboard::get_clipboard_provider;
use std::io::Write;
...
pub fn general() -> std::io::Result<()> {
    ...
    let config_file = rustcode_loader::config_file();
    let lang_file = rustcode_loader::lang_config_file();
    let log_file = rustcode_loader::log_file();
    let rt_dirs = rustcode_loader::runtime_dirs();
...
pub fn clipboard() -> std::io::Result<()> {
    ...
    match board.name().as_ref() {
        "none" => {
            ...
            writeln!(stdout, "    {}",
                "https://github.com/rustcode-editor/rustcode/wiki/Troubleshooting#copypaste-fromto-system-clipboard-not-working"
            .red().underlined())?;
        }
...
pub fn languages_all() -> std::io::Result<()> {
    ...
    let check_binary = |cmd: Option<&str>| match cmd {
        Some(cmd) => match rustcode_stdx::env::which(cmd) {
...
fn probe_protocols<'a, I: Iterator<Item = &'a str> + 'a>(
    ...
    for cmd in server_cmds {
        let (path, icon) = match rustcode_stdx::env::which(cmd) {
...
fn probe_protocol(protocol_name: &str, server_cmd: Option<String>) -> std::io::Result<()> {
    ...
    if let Some(cmd) = server_cmd {
        let path = match rustcode_stdx::env::which(&cmd) {
