...
impl EditorView {
    ...
    pub fn doc_diagnostics_highlights(
        ...
        use rustcode_core::diagnostic::{DiagnosticTag, Range, Severity};
    ...
    pub fn highlight_focused_view_elements(
        ...
        if let Some(syntax) = doc.syntax() {
            let text = doc.text().slice(..);
            use rustcode_core::match_brackets;
    ...
    fn handle_keymap_event(
        ...
        let mut execute_command = |command: &commands::MappableCommand| {
            command.execute(cxt);
            rustcode_event::dispatch(PostCommand { command, cx: cxt });

            let current_mode = cxt.editor.mode();
            if current_mode != last_mode {
                rustcode_event::dispatch(OnModeSwitch {
                    old_mode: last_mode,
...
impl Component for EditorView {
    ...
    fn render(&mut self, area: Rect, surface: &mut Surface, cx: &mut Context) {
        ...
        use rustcode_view::editor::BufferLine;
        ...
        if let Some((status_msg, severity)) = &cx.editor.status_msg {
            ...
            use rustcode_view::editor::Severity;
        ...
        if area.width.saturating_sub(status_msg_width as u16) > key_width {
            ...
            if let Some((reg, _)) = cx.editor.macro_recording {
                let disp = format!("[{}]", reg);
                let style = style
                    .fg(rustcode_view::graphics::Color::Yellow)
