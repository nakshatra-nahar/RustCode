                                    ...
                                    let call: job::Callback = Callback::EditorCompositor(Box::new(
                                        move |_editor: &mut Editor, compositor: &mut Compositor| {
                                            ...
                                            let popup = Popup::new("invalid-regex", contents)
                                                .position(Some(rustcode_core::Position::new(
...
pub fn file_picker(root: PathBuf, config: &rustcode_view::editor::Config) -> Picker<PathBuf> {
    ...
    walk_builder.add_custom_ignore_filename(rustcode_loader::config_dir().join("ignore"));
...
pub mod completers {
    ...
    use rustcode_core::syntax::LanguageServerFeature;
    use rustcode_view::document::SCRATCH_BUFFER_NAME;
    use rustcode_view::theme;
    use rustcode_view::{editor::Config, Editor};
    ...
    pub fn theme(_editor: &Editor, input: &str) -> Vec<Completion> {
        let mut names = theme::Loader::read_names(&rustcode_loader::config_dir().join("themes"));
        for rt_dir in rustcode_loader::runtime_dirs() {
        ...
        F: Fn(&ignore::DirEntry) -> FileMatch,
    {
        ...
        let is_tilde = input == "~";
        let path = rustcode_stdx::path::expand_tilde(Path::new(input));
        ...
        let (dir, file_name) = if input.ends_with(std::path::MAIN_SEPARATOR) {
            ...
            let path = if is_period {
                ...
                match path.parent() {
                    ...
                    _ => Cow::Owned(rustcode_stdx::env::current_working_dir()),
                }
            };
