...
use dap::StackFrame;
use rustcode_vcs::DiffProviderRegistry;

use futures_util::stream::select_all::SelectAll;
use futures_util::{future, StreamExt};
use rustcode_lsp::{Call, LanguageServerId};
...
pub fn get_terminal_provider() -> Option<TerminalConfig> {
    use rustcode_stdx::env::binary_exists;
...
pub fn get_terminal_provider() -> Option<TerminalConfig> {
    use rustcode_stdx::env::{binary_exists, env_var_is_set};
...
pub struct Editor {
    ...
    pub language_servers: rustcode_lsp::Registry,
...
impl Editor {
    pub fn new(
        ...
        let language_servers = rustcode_lsp::Registry::new(syn_loader.clone());
    ...
    pub fn move_path(&mut self, old_path: &Path, new_path: &Path) -> io::Result<()> {
        ...
        for language_server in language_servers {
            ...
            let edit = match rustcode_lsp::block_on(request) {
                Ok(edit) => edit,
                Err(err) => {
        ...
        let language_servers = lang.as_ref().map_or_else(HashMap::default, |language| {
            self.language_servers
                ...
                .filter_map(|(lang, client)| match client {
                    ...
                    Err(err) => {
                        if let rustcode_lsp::Error::ExecutableNotFound(err) = err {
    ...
    pub fn new_file_from_stdin(&mut self, action: Action) -> Result<DocumentId, Error> {
        ...
        doc.ensure_view_init(view.id);
        let transaction =
            rustcode_core::Transaction::insert(doc.text(), doc.selection(view.id), stdin.into())
    ...
    pub fn open(&mut self, path: &Path, action: Action) -> Result<DocumentId, Error> {
        let path = rustcode_stdx::path::canonicalize(path);
    ...
    pub fn doc_diagnostics<'a>(
        language_servers: &'a rustcode_lsp::Registry,
        diagnostics: &'a BTreeMap<PathBuf, Vec<(lsp::Diagnostic, LanguageServerId)>>,
        document: &Document,
    ) -> impl Iterator<Item = rustcode_core::Diagnostic> + 'a {
    ...
    pub async fn wait_event(&mut self) -> EditorEvent {
        ...
        loop {
            tokio::select! {
                ...
                _ = rustcode_event::redraw_requested() => {
