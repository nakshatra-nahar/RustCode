...
use rustcode_core::Position;
use rustcode_view::graphics::{CursorKind, Rect};
...
pub use rustcode_view::input::Event;
...
impl<'a> Context<'a> {
    ...
    pub fn block_try_flush_writes(&mut self) -> anyhow::Result<()> {
        tokio::task::block_in_place(|| rustcode_lsp::block_on(self.jobs.finish(self.editor, None)))?;
        tokio::task::block_in_place(|| rustcode_lsp::block_on(self.editor.flush_writes()))?;
        Ok(())
    }
}
...
pub trait AnyComponent {
    ...
    ///
    /// ```rust
    /// use rustcode_term::{ui::Text, compositor::Component};
