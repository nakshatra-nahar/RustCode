...
impl PathOrId {
    fn get_canonicalized(self) -> Self {
        use PathOrId::*;
        match self {
            Path(path) => Path(rustcode_stdx::path::canonicalize(path)),
            Id(id) => Id(id),
        }
    }
}
...
impl<T: Item + 'static> Picker<T> {
    pub fn stream(editor_data: T::Data) -> (Nucleo<T>, Injector<T>) {
        let matcher = Nucleo::new(
            Config::DEFAULT,
            Arc::new(rustcode_event::request_redraw),
            None,
            1,
        );
    ...
    fn render_preview(&mut self, area: Rect, surface: &mut Surface, cx: &mut Context) {
        ...
        if let Some((path, range)) = self.current_file(cx.editor) {
            ...
            for spans in EditorView::doc_diagnostics_highlights(doc, &cx.editor.theme) {
                ...
                overlay_highlights = Box::new(rustcode_core::syntax::merge(overlay_highlights, spans));
            }
