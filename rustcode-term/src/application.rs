...
use rustcode_core::{diagnostic::Severity, pos_at_coords, syntax, Selection};
use rustcode_lsp::{
    lsp::{self, notification::Notification},
    util::lsp_range_to_range,
    LanguageServerId, LspProgressMap,
};
use rustcode_stdx::path::get_relative_path;
use rustcode_view::{
    align_view,
...
fn setup_integration_logging() {
    let level = std::env::var("rustcode_LOG_LEVEL")
...
impl Application {
    pub fn new(args: Args, config: Config, lang_loader: syntax::Loader) -> Result<Self, Error> {
        ...
        use rustcode_view::editor::Action;

        let mut theme_parent_dirs = vec![rustcode_loader::config_dir()];
        theme_parent_dirs.extend(rustcode_loader::runtime_dirs().iter().cloned());
        ...
        if args.load_tutor {
            let path = rustcode_loader::runtime_file(Path::new("tutor"));
            ...
            tokio::select! {
                ...
                Some(msg) = self.jobs.status_messages.recv() => {
                    let severity = match msg.severity{
                        rustcode_event::status::Severity::Hint => Severity::Hint,
                        rustcode_event::status::Severity::Info => Severity::Info,
                        rustcode_event::status::Severity::Warning => Severity::Warning,
                        rustcode_event::status::Severity::Error => Severity::Error,
                    };
                    // TODO: show multiple status messages at once to avoid clobbering
                    self.editor.status_msg = Some((msg.message, severity));
                    rustcode_event::request_redraw();
                }
    ...
    fn refresh_language_config(&mut self) -> Result<(), Error> {
        let lang_loader = rustcode_core::config::user_lang_loader()?;
    ...
    pub async fn handle_signals(&mut self, signal: i32) -> bool {
        match signal {
            signal::SIGTSTP => {
                ...
                // SAFETY:
                //
                // - rustcode must have permissions to send signals to all processes in its signal
                ...
                let res = unsafe {
                    ...
                    // as mentioned above, the terminal will get stuck if `rustcode` was spawned from
                    // an external process and that process waits for `rustcode` to complete. This may
    ...
    pub async fn handle_language_server_message(
        &mut self,
        call: rustcode_lsp::Call,
        server_id: LanguageServerId,
    ) {
        use rustcode_lsp::{Call, MethodCall, Notification};
        ...
        match call {
            Call::Notification(rustcode_lsp::jsonrpc::Notification { method, params, .. }) => {
                let notification = match Notification::parse(&method, params) {
                    Ok(notification) => notification,
                    Err(rustcode_lsp::Error::Unhandled) => {
                ...
                match notification {
                    ...
                    Notification::PublishDiagnostics(mut params) => {
                        let path = match params.uri.to_file_path() {
                            Ok(path) => rustcode_stdx::path::normalize(&path),
                            Err(_) => {
                    ...
                    Notification::Exit => {
                        ...
                        // LSPs may produce diagnostics for files that haven't been opened in rustcode,
            ...
            Call::MethodCall(rustcode_lsp::jsonrpc::MethodCall {
                method, params, id, ..
            }) => {
                let reply = match MethodCall::parse(&method, params) {
                    Err(rustcode_lsp::Error::Unhandled) => {
                        ...
                        Err(rustcode_lsp::jsonrpc::Error {
                            code: rustcode_lsp::jsonrpc::ErrorCode::MethodNotFound,
                    ...
                    Err(err) => {
                        ...
                        Err(rustcode_lsp::jsonrpc::Error {
                            code: rustcode_lsp::jsonrpc::ErrorCode::ParseError,
                    ...
                    Ok(MethodCall::ApplyWorkspaceEdit(params)) => {
                        ...
                        if language_server.is_initialized() {
                            ...
                            Err(rustcode_lsp::jsonrpc::Error {
                                code: rustcode_lsp::jsonrpc::ErrorCode::InvalidRequest,
    ...
    fn handle_show_document(
        ...
        let action = match take_focus {
            Some(true) => rustcode_view::editor::Action::Replace,
            _ => rustcode_view::editor::Action::VerticalSplit,
        };
    ...
    fn restore_term(&mut self) -> std::io::Result<()> {
        let terminal_config = self.config.load().editor.clone().into();
        use rustcode_view::graphics::CursorKind;
        self.terminal
            .backend_mut()
