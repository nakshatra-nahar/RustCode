...
use rustcode_core::{chars, Change, Transaction};
use rustcode_view::{graphics::Rect, Document, Editor};

use crate::ui::{menu, Markdown, Menu, Popup, PromptEvent};

use rustcode_lsp::{lsp, util, LanguageServerId, OffsetEncoding};
...
impl Completion {
    ...
    fn resolve_completion_item(
        language_server: &rustcode_lsp::Client,
        ...
        let response = rustcode_lsp::block_on(future);
        match response {
            Ok(item) => Some(item),
            Err(err) => {
