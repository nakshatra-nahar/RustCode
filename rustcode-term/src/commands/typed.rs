...
use rustcode_core::fuzzy::fuzzy_match;
use rustcode_core::indent::MAX_INDENT;
use rustcode_core::{line_ending, shellwords::Shellwords};
use rustcode_view::document::{read_to_string, DEFAULT_LANGUAGE_NAME};
use rustcode_view::editor::{CloseError, ConfigEvent};
...
fn open(cx: &mut compositor::Context, args: &[Cow<str>], event: PromptEvent) -> anyhow::Result<()> {
    ...
    for arg in args {
        ...
        let path = rustcode_stdx::path::expand_tilde(path);
...
fn set_line_ending(
    ...
    let transaction = Transaction::change(
        ...
        doc.text().lines().filter_map(|line| {
            pos += line.len_chars();
            match rustcode_core::line_ending::get_line_ending(&line) {
...
fn change_current_directory(
    ...
    let dir = args
        ...
        .as_ref();
    let dir = rustcode_stdx::path::expand_tilde(Path::new(dir));

    rustcode_stdx::env::set_current_working_dir(dir)?;

    cx.editor.set_status(format!(
        "Current working directory is now {}",
        rustcode_stdx::env::current_working_dir().display()
    ));
    Ok(())
}
...
fn lsp_workspace_command(
    ...
    if args.is_empty() {
        let commands = options
            .commands
            .iter()
            .map(|command| rustcode_lsp::lsp::Command {
                title: command.clone(),
...
fn tree_sitter_highlight_name(
    ...
    ) -> Option<rustcode_core::syntax::Highlight> {
        use rustcode_core::syntax::HighlightEvent;
...
fn debug_eval(
    ...
    if let Some(debugger) = cx.editor.debugger.as_mut() {
        ...
        let response = rustcode_lsp::block_on(debugger.eval(args.join(" "), Some(frame_id)))?;
...
fn reflow(
    ...
    let transaction = Transaction::change_by_selection(rope, selection, |range| {
        ...
        let reflowed_text = rustcode_core::wrap::reflow_hard_wrap(&fragment, text_width);
...
fn tree_sitter_subtree(
    ...
    if let Some(syntax) = doc.syntax() {
        ...
        if let Some(selected_node) = syntax.descendant_for_byte_range(from, to) {
            ...
            rustcode_core::syntax::pretty_print_tree(&mut contents, selected_node)?;
            contents.push_str("\n```");
...
fn open_config(
    ...
    cx.editor
        .open(&rustcode_loader::config_file(), Action::Replace)?;
    Ok(())
}
...
fn open_workspace_config(
    ...
    cx.editor
        .open(&rustcode_loader::workspace_config_file(), Action::Replace)?;
    Ok(())
}
...
fn open_log(
    ...
    cx.editor.open(&rustcode_loader::log_file(), Action::Replace)?;
    Ok(())
}
            ...
            move |editor: &mut Editor, compositor: &mut Compositor| {
                if !output.is_empty() {
                    ...
                    let popup = Popup::new("shell", contents).position(Some(
                        rustcode_core::Position::new(editor.cursor().0.unwrap_or_default().row, 2),
                    ));
...
fn yank_diagnostic(
    ...
    let diag: Vec<_> = doc
        .diagnostics()
        .iter()
        .filter(|d| primary.overlaps(&rustcode_core::Range::new(d.range.start, d.range.end)))
...
pub const TYPABLE_COMMAND_LIST: &[TypableCommand] = &[
    ...
    TypableCommand {
        name: "log-open",
        aliases: &[],
        doc: "Open the rustcode log file.",
        fun: open_log,
        signature: CommandSignature::none(),
    },
    TypableCommand {
