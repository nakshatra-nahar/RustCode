...
use rustcode_core::syntax::{DebugArgumentValue, DebugConfigCompletion, DebugTemplate};
use rustcode_dap::{self as dap, Client};
use rustcode_lsp::block_on;
use rustcode_view::editor::Breakpoint;
...
use rustcode_view::handlers::dap::{breakpoints_changed, jump_to_stack_frame, select_thread_id};
...
fn dap_callback<T, F>(
    jobs: &mut Jobs,
    call: impl Future<Output = rustcode_dap::Result<serde_json::Value>> + 'static + Send,
    callback: F,
) where
...
pub fn dap_start_impl(
    ...
    args.insert("cwd", to_value(rustcode_stdx::env::current_working_dir())?);
...
pub fn dap_variables(cx: &mut Context) {
    ...
    for scope in scopes.iter() {
        // use rustcode_view::graphics::Style;
        use tui::text::Span;
